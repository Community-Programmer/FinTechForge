...
export async function getLearningPathById(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const path = await prisma.learningPath.findUnique({ where: { id }, include: { lessons: true } });
...
export async function createLearningPath(req: Request, res: Response) {
  try {
    ...
    const path = await prisma.learningPath.create({ data: { title, color, icon } });
...
export async function updateLearningPath(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const path = await prisma.learningPath.update({ where: { id }, data: req.body });
    res.status(200).json({ path });
  } catch (error) {
